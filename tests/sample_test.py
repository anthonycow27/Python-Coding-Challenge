...
class TestServiceFunnelScrape(unittest.TestCase):
    ...
    def test_valid_tags_with_snippet(self):
        ...
        self.assertEqual(result["snippet"], json.dumps(
    ...
    def test_path_relevance(self):
        self.setUp()
        result = self.service_funnel.handle_request(
            {"selected_tags": [{"name": "A"}, {"name": "B"}, {"name": "C"}]}
        )
        result2 = self.service_funnel.handle_request(
            {"selected_tags": [{"name": "C"}, {"name": "B"}, {"name": "A"}]}
        )
        self.assertEqual(result['status'], result2['status'])
        self.assertEqual(result['snippet'], result2['snippet'])
